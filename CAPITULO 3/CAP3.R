...
#DENSITY DATA-----
library(readr)
density <- read_csv(file.path(Data, "density.csv"))#read density data
metadata <- read_csv(file.path(Data,"metadata.csv"))#read metadata
...
write.csv(checklist, file = "checklist.csv")
...
labelsRobot <- colnames(dfRobot)[21:ncol(dfRobot)]
labelsHuman <- colnames(dfHuman)[21:ncol(dfHuman)]
...
# GLM BC -------
library(lme4)
model = glmer(BC ~DataSet + `reef area`+ DataSet*`reef area` + (1|`reef name`/Quadrat), data=BC_RH_1, family=binomial)
model = glmer(BC ~DataSet + `reef name`+ DataSet*`reef name` + (1|`reef name`/`reef area`), data=BC_RH_1, family=binomial)

model = glm(BC ~DataSet + `reef area`+ `reef name` , data=BC_RH_1, family=binomial)
summary(model)
...
# Cargar el paquete nnet si aún no está cargado
library(nnet)

# Ajustar el modelo de regresión logística multinomial
modelo_multinomial <- multinom(Label.robot ~ Label.human, data = annotation.human.robot)
...
modelo <- glm(Label.robot ~ Label.human, data = annotation.human.robot, family = binomial)
